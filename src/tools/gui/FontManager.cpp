...
void FontManager::init(const FontManagerConfig* config)
{
    ...
    for( int each_slot = 0; each_slot < FontSlot_COUNT; ++each_slot )
    {
        ...
        {
            TOOLS_LOG(TOOLS_WARNING, "NodableView", "No default text_font declared for slot #%i, using ImGui's default text_font as fallback\n", each_slot);
...
ImFont* FontManager::load_font(const FontConfig& font_config)
{
    ...
    if (font_config.icons_enable )
    {
        ...
        {
            TOOLS_LOG(TOOLS_WARNING, "NodableView", "config().font_manager.icon.path is empty, icons will be \"?\"\n");
            return font;
        }
